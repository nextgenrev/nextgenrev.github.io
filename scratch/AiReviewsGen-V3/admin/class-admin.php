...
class Admin {
    ...
    public function enqueue_assets($hook) {
        // Load on our plugin pages AND the post editor (for GitHub metabox)
        $is_plugin_page = in_array($hook, array('toplevel_page_ai-gen', 'settings_page_ai-gen-settings'));
    ...
    /**
     * Register the GitHub Pages metabox on the post edit screen.
     *
     * @since 8.1.0
     * @return void
     */
    public function add_github_metabox() {
        add_meta_box(
            'airg_github_publish',
            '<i class="fab fa-github" style="margin-right:5px;"></i> Publish to GitHub Pages',
            array($this, 'render_github_metabox'),
            'post',
            'side',
            'high'
        );
    ...
    /**
     * Render the GitHub Pages metabox content.
     *
     * @since 8.1.0
     * @param \WP_Post $post The current post.
     * @return void
     */
    public function render_github_metabox($post) {
        $options = get_option($this->option_name, array());
        $has_github = !empty($options['github_token']) && !empty($options['github_repo']);
        $pushed_at = get_post_meta($post->ID, '_airg_github_pushed', true);

        wp_nonce_field('airg_github_push_nonce', 'airg_github_nonce');
        ?>
        <div id="airg-github-metabox" style="text-align:center;">
            <?php if (!$has_github): ?>
                <p style="color:#d63638; font-size:12px; margin:8px 0;"><i class="fas fa-exclamation-triangle"></i> GitHub token/repo not configured. <a href="<?php echo admin_url('options-general.php?page=ai-gen-settings'); ?>">Settings</a></p>
            <?php else: ?>
                <?php if ($pushed_at): ?>
                    <p style="color:#00a32a; font-size:12px; margin:8px 0;"><i class="fas fa-check-circle"></i> Last pushed: <?php echo esc_html($pushed_at); ?></p>
                <?php endif; ?>
                <button type="button" id="airg-github-push-btn" class="button button-primary" style="width:100%; margin:6px 0; padding:6px 0;" data-post-id="<?php echo esc_attr($post->ID); ?>">
                    <i class="fab fa-github"></i> <?php echo $pushed_at ? 'Update on GitHub' : 'Publish to GitHub'; ?>
                </button>
                <div id="airg-github-status" style="margin-top:8px; font-size:12px;"></div>
            <?php endif; ?>
        </div>
        <script>
        jQuery(function($){
            $('#airg-github-push-btn').on('click', function(e){
                e.preventDefault();
                var btn = $(this);
                var statusEl = $('#airg-github-status');
                btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Pushing...');
                statusEl.html('').css('color', '#666');
                $.ajax({
                    url: ajaxurl,
                    type: 'POST',
                    data: {
                        action: 'airg_push_github',
                        nonce: $('#airg_github_nonce').val(),
                        post_id: btn.data('post-id')
                    },
                    timeout: 60000,
                    success: function(res){
                        if (res.success) {
                            statusEl.html('<i class="fas fa-check-circle"></i> ' + (res.data.message || 'Published!')).css('color', '#00a32a');
                            btn.html('<i class="fab fa-github"></i> Update on GitHub');
                        } else {
                            statusEl.html('<i class="fas fa-times-circle"></i> ' + (res.data || 'Failed')).css('color', '#d63638');
                        }
                        btn.prop('disabled', false);
                    },
                    error: function(){
                        statusEl.html('<i class="fas fa-times-circle"></i> Request failed').css('color', '#d63638');
                        btn.prop('disabled', false);
                    }
                });
            });
    ...
    /**
     * AJAX handler: push a post to GitHub Pages.
     *
     * @since 8.1.0
     * @return void
     */
    public function ajax_push_github() {
        check_ajax_referer('airg_github_push_nonce', 'nonce');
        ...
        if ($post_id <= 0 || !get_post($post_id)) {
            wp_send_json_error(__('Invalid post.', 'ai-review-generator-pro'), 400);
        }

        $options = get_option($this->option_name, array());
        $pusher = new \AIReviewGenerator\GitHubPusher($options);
        $result = $pusher->push($post_id, get_the_title($post_id));

        if (is_wp_error($result)) {
            wp_send_json_error($result->get_error_message(), 500);
        }

        wp_send_json_success(array('message' => __('Successfully published to GitHub Pages!', 'ai-review-generator-pro')));
    }
}
